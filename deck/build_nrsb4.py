...
def cover(wg,name,chips_,idx,total):
    ...
    if isinstance(chips_[0],tuple):
        cw=I(2.62); gap=I(0.14); y=I(4.65)
        ...
        for i,t in enumerate(chips_):
            icon_disc(s,["cpu","file-search","handshake"][i%3],ML,y+i*I(0.62),I(0.4),BR_T,"#4A35FF")
            text(s,ML+I(0.56),y+i*I(0.62)+I(0.05),I(7.6),I(0.4),t,font=HEAD,size=11.5,bold=True,color=INK,ls=1.1)
...
def opendata(idx,total,wg):
    ...
    header(s,"02 · What we have built, open public data",
           ...
           lead="Screenshots taken from the live platforms this week. Each one turns a scattered public record into structured, citable evidence.",
           sect=(2,"WHAT WE HAVE BUILT"))
    photo(s,a5("parliament_live.png"),ML,I(2.12),I(6.0),I(3.4),fill=True,anchor=0.0,
          caption="Road Safety in Parliament: all 1,444 questions since 2014 across four ministries; 275 of 540 sitting Lok Sabha MPs have never asked one; 92% answered only in writing.")
    photo(s,a5("sctracker.png"),I(6.72),I(2.12),I(6.0),I(3.4),fill=True,anchor=0.0,
    ...
    rows=[("database","Road-Infrastructure Defect Repository","An AI pipeline reads local-language news from 146 districts every morning and turns defect reports into structured, mapped evidence.",TE_T,"#0E8A8A"),
          ("bar-chart-3","Crash Data Dashboard","MoRTH's annual 'Road Accidents in India' books as a usable product: report cards for 36 states and 50 cities, open licence (CC-BY).",AM_T,"#E58900")]
    y=I(6.05)
    for j,(ic,t,d,tint,col) in enumerate(rows):
        x=ML+j*I(6.1)
        icon_disc(s,ic,x,y,I(0.4),tint,col)
        text(s,x+I(0.52),y-I(0.03),I(5.5),I(0.35),t,font=HEAD,size=10,bold=True,color=INK,ls=1.0)
        text(s,x+I(0.52),y+I(0.26),I(5.4),I(0.6),d,size=8.5,color=MUT,ls=1.18)
    footer(s,wg,idx,total)
    return s
...
def wg5_body(wg,total,start):
    ...
    bars=[("~30,000","hit-and-run deaths every year (MoRTH)",I(3.1),INK),
          ("205","compensation claims filed, FY23",I(0.35),BRAND),
          ("95","claims settled (Rs 1.78 crore paid)",I(0.3),AMBER)]
    yb=I(2.75)
    for v,l,w,col in bars:
        ...
        yb=yb+I(0.75)
    text(s,I(9.25),I(5.0),I(3.25),I(0.5),"Fewer than 1 in 100 losses become claims. Source: Lok Sabha reply AU6419.",size=8.5,color=MUT,ls=1.2,italic=True)
...
def build_wg2():
    ...
    why_fit(2,6,TOTAL,
        ...
        smes=[("Prof. Geetam Tiwari","TRIP Centre, IIT Delhi · IRC H-7","x_gtiwari.png",True),
              ("Abhay Damle","Ex-Joint Secretary, MoRTH","x_damle.png",True),
              ("Vaibhav Dange","Ex-Advisor, MoRTH","x_dange.png",True),
    ...
    close(2,27,TOTAL)
    save(os.path.join(BUILD,"CFI_NRSB_WG2_v4.pptx"))

def build_wg5():
    new_deck(); TOTAL=26
    cover(5,"Post-Crash Care, Policy Standards & State Implementation",
          ["Technology that helps crash victims: Aasha, calculator, helpdesks",
           "Policy research and open evidence on post-crash care",
           "Working hand in hand with authorities in Delhi, Gurugram and Rajasthan"],1,TOTAL)
    letter(5,
    ...
    why_fit(5,6,TOTAL,
        ...
        smes=[("Justice J.R. Midha","Retd., Delhi HC · MACT process architect","x_pp_midha.png",True),
              ...
              ("Dr. Sushma Sagar","Prof. & In-charge, Trauma Surgery, JPN Apex Trauma Centre, AIIMS","SS",False),
    ...
    close(5,26,TOTAL)
    save(os.path.join(BUILD,"CFI_NRSB_WG5_v4.pptx"))
