...
def about(idx,total,wg):
    ...
    if wg==2:
        boxmsg=[("Everything in this deck serves the Group's agenda: ",{'bold':True,'color':BRAND}),
                ("safer road engineering, managed speeds, and vulnerable road users protected by design.",{'color':BRAND})]
    else:
        boxmsg=[("Everything in this deck serves the Group's agenda: ",{'bold':True,'color':BRAND}),
                ("faster compensation, stronger EMS evidence, and support victims can actually use.",{'color':BRAND})]
...
def team(idx,total,wg):
    ...
    core=[("t_akhtar.png","Akhtar Hussain","Mission Lead","IIT Delhi · ex-Bain · strategy, programmes, new initiatives, partnerships and product"),
          ...
          ("t_krishna.png","Krishna","Tech & Product","Builds and runs Aasha, SATARK and the open public data platforms"),
...
def wg2_body(wg,total,start):
    ...
    gt=[("45%","of India's road deaths are two-wheeler riders (MoRTH)"),
        ("80%+","of gig riders we surveyed ride 10+ hours a day"),
        ("~70%","of families at our legal helpdesks are VRU households")]
    ...
    text(s,ML,I(3.98),I(7.5),I(0.6),
         "Forty pages, 87 references, built from a 300+ rider study across Delhi NCR. The study lands on two first levers: rider training and helmet compliance, with delivery-incentive design close behind, and has been shared with platforms and police.",
...
def build_wg2():
    ...
    why_fit(2,6,TOTAL,
        ...
        smes=[("Dr. Richa Ahuja","Asst Professor, IIT Kharagpur","x_richa.png",True),
              ...
              ("Sarika Panda Bhatt","Co-founder, Raahgiri Day","x_sarika.png",True)])
    portfolio(7,TOTAL,2)
    satark(8,TOTAL,2)
    opendata(9,TOTAL,2)
    ...
    close(2,28,TOTAL)
    save(os.path.join(BUILD,"CFI_NRSB_WG2_v6.pptx"))
...
def build_wg5():
    ...
    close(5,26,TOTAL)
    save(os.path.join(BUILD,"CFI_NRSB_WG5_v6.pptx"))
