...
def letter(wg,p3,idx,total):
    ...
    if wg==2:
        p1=("Crashfree India is a young road safety nonprofit, registered as the Vision Zero Trust, co-founded with the Indian Road Safety Council, and backed by a three-year, US$3 million commitment from Cars24. "
            "Our core work is road infrastructure: trained student teams audit dangerous locations under senior experts, to IRC codes and MoRTH checklists, and we walk each recommended fix through the road-owning authority until it is built, tracked on a public dashboard. MS Dhoni carries this message to the country as our Goodwill Ambassador.")
    elif wg==3:
        p1=("Crashfree India is a young road safety nonprofit, registered as the Vision Zero Trust, co-founded with the Indian Road Safety Council, and backed by a three-year, US$3 million commitment from Cars24. "
            "Our work meets this Working Group where the risk is growing fastest: two-wheeler riders, the commercial and gig fleets they increasingly ride for, and the standards and technology that keep vehicles safe and compliant. Through Cars24 we also carry an industry vantage point into vehicle markets, fleet operations and used-vehicle certification. MS Dhoni carries this message to the country as our Goodwill Ambassador.")
    else:
        p1=("Crashfree India is a young road safety nonprofit, registered as the Vision Zero Trust, co-founded with the Indian Road Safety Council, and backed by a three-year, US$3 million commitment from Cars24. "
            "Our core work in post-crash care starts with families: our research maps where compensation stalls, our tools stand with victims from the first hours after a crash, and our teams sit inside district machinery to help files move. MS Dhoni carries this message to the country as our Goodwill Ambassador.")
...
def agenda(wg,sect4,r4,r5,idx,total):
    ...
    hline(s,ML,I(0.62),I(0.55),color=BRAND,wt=2.0)
    text(s,ML,I(0.76),I(12),I(0.5),[("Five sections, ",{'color':INK}),("from who we are to what we offer",{'color':BRAND})],font=HEAD,size=22.5,bold=True,ls=1.1)
...
def about(idx,total,wg):
    ...
    if wg==2:
        boxmsg=[("The Group's mandate is our daily work: ",{'bold':True,'color':BRAND}),
                ("audits that end in built fixes, evidence for managing speeds, and streets designed around pedestrians and riders.",{'color':BRAND})]
    elif wg==3:
        boxmsg=[("The Group's mandate is our daily work: ",{'bold':True,'color':BRAND}),
                ("rider-risk research, vehicle-standards engagement with the people who wrote them, and compliance technology proven with police.",{'color':BRAND})]
    else:
        boxmsg=[("The Group's mandate is our daily work: ",{'bold':True,'color':BRAND}),
                ("compensation pipelines that move, evidence for emergency-care standards, and support families can actually use.",{'color':BRAND})]
    text(s,ML+I(0.25),I(5.72),I(7.1),I(0.8),boxmsg,size=11.5,ls=1.3)
...
def approach(idx,total,wg):
    ...
    steps=[("search-check","1 · Evidence","Field audits, file reviews and research to official standards.","31 sites audited · 50+ stakeholder consultations",GR_T,"#0A9955"),
           ...
           ("cpu","3 · Technology","Dashboards, AI and WhatsApp tools that fit how officials already work.","Aasha · SATARK · 4 open data platforms",TE_T,"#0E8A8A"),
           ("route","4 · Follow-through","Every recommendation tracked publicly to completion.","Implementation started at 15 sites · 7 fully built",AM_T,"#E58900")]
    tw=I(2.92); gx=I(0.13)
...
def why_fit(wg,idx,total,focus,bring,smes,photo_card=None):
    ...
    for nm,role,img,isphoto in smes:
        ...
        text(s,x+I(1.06),I(5.45),cw_-I(1.15),I(1.2),
             [(nm,{'bold':True,'size':9,'color':INK}),("\n"+role,{'size':7.5,'color':MUT})],
             font=HEAD,ls=1.12,anchor=MSO_ANCHOR.MIDDLE)
        x=x+cw_+I(0.09)
    footer(s,wg,idx,total)
    return s
...
def opendata(idx,total,wg):
    ...
    header(s,"02 · What we have built, open public data",
           ...
           lead="Four platforms, each turning a scattered public record into structured, citable evidence. The Parliament tracker is shown live, as captured this week.",
           sect=(2,"WHAT WE HAVE BUILT"))
    ...
    dx=I(6.72); dw=I(6.0)
    rect(s,dx,I(2.12),dw,I(2.62),fill=PAPER,line=LINE,rounded=True,radius=0.06)
    icon_disc(s,"database",dx+I(0.2),I(2.32),I(0.46),TE_T,"#0E8A8A")
    text(s,dx+I(0.8),I(2.3),I(4.2),I(0.55),"National Road-Infrastructure\nDefect Repository",font=HEAD,size=12.5,bold=True,color=INK,ls=1.1)
    chip(s,dx+dw-I(1.55),I(2.34),I(1.38),I(0.34),"DAILY · 07:00 IST",fill=TE_T,color=TEAL,size=7)
    text(s,dx+I(0.2),I(3.06),dw-I(0.4),I(0.5),
         "An AI pipeline reads local-language news every morning and turns road-defect reports into structured, mapped, ranked evidence, each entry carrying its verbatim source.",size=8.5,color=MUT,ls=1.2)
    dts=["Potholes","Missing signage","Unlit stretches","Blind curves","Broken dividers"]
    for j,t_ in enumerate(dts):
        chip(s,dx+I(0.2)+j*I(1.14),I(3.62),I(1.06),I(0.3),t_,fill=MIST,color=INK,size=6.5,bold=False)
    sts=[("146","districts scanned daily"),("13","languages read"),("21","defect codes tracked")]
    for j,(v,l) in enumerate(sts):
        x=dx+I(0.2)+j*I(1.92)
        rect(s,x,I(4.04),I(1.8),I(0.56),fill=TE_T,rounded=True,radius=0.12)
        text(s,x+I(0.1),I(4.1),I(0.52),I(0.3),v,font=HEAD,size=12,bold=True,color=TEAL,ls=1.0)
        text(s,x+I(0.62),I(4.09),I(1.15),I(0.44),l,size=6.5,color=MUT,ls=1.08)
    text(s,dx,I(4.79),dw,I(0.3),"Local news → AI extraction → confidence gate → human review → public map. Live across Bihar, UP and Rajasthan districts today.",size=8.5,color=MUT,ls=1.1)
...
def victim_suite(idx,total,wg):
    ...
    header(s,"02 · What we have built, victim support",
           ...
           sect=(2,"WHAT WE HAVE BUILT"))
    cols=[("bot","Aasha chatbot","India's first AI legal companion for crash victims, on WhatsApp and voice, no app needed.",a6("ui_aasha_chat.png"),BR_T,"#4A35FF"),
          ("calculator","Compensation Calculator","Free instant estimates computed on Supreme Court principles (Sarla Verma, Pranay Sethi).",a6("ui_mact_calc.png"),TE_T,"#0E8A8A"),
          ("book-open","'Meet Aasha' comics","Plain-language legal education that families actually read, at desks and in schools.",a6("comic_aasha_page.png"),AM_T,"#E58900"),
          ("languages","Hindi rights guide","All three compensation routes, NALSA 15100 and the Cashless Treatment Scheme 2025.",a3("guide_p1.png"),GR_T,"#0A9955")]
    cw_=I(2.93); gx=I(0.12)
    for j,(ic,t,d,img,tint,col) in enumerate(cols):
        x=ML+j*(cw_+gx)
        rect(s,x,I(2.1),cw_,I(4.15),fill=PAPER,line=LINE,rounded=True,radius=0.06)
        photo_fit(s,img,x+I(0.12),I(2.24),cw_-I(0.24),I(2.5),border=False,align='center')
        icon_disc(s,ic,x+I(0.14),I(4.95),I(0.38),tint,col)
        text(s,x+I(0.62),I(4.99),cw_-I(0.74),I(0.32),t,font=HEAD,size=10.5,bold=True,color=INK,ls=1.0)
        text(s,x+I(0.14),I(5.42),cw_-I(0.28),I(0.75),d,size=8.5,color=MUT,ls=1.22)
    rect(s,ML,I(6.42),CW,I(0.5),fill=GR_T,rounded=True,radius=0.14)
    text(s,ML+I(0.22),I(6.50),CW-I(0.45),I(0.36),
         [("Hospital helpdesks bring all of this to the trauma-ward bedside ",{'bold':True,'color':GREEN}),
          (", 100+ victims supported in the first four days, run with DLSA and Neev Foundation.",{'color':GREEN})],size=9.5,ls=1.2)
    footer(s,wg,idx,total)
    return s
...
def team(idx,total,wg):
    ...
    for img,nm,cred in trustees:
        rect(s,x,I(2.06),I(3.97),I(0.94),fill=MIST,rounded=True,radius=0.10)
        circp(s,img,x+I(0.13),I(2.18),I(0.7))
        text(s,x+I(0.98),I(2.15),I(2.9),I(0.32),nm,font=HEAD,size=10.5,bold=True,color=INK)
        text(s,x+I(0.98),I(2.45),I(2.9),I(0.55),cred,size=8,color=MUT,ls=1.15)
        x=x+I(4.06)
    text(s,ML,I(3.22),I(12),I(0.26),"THE FULL-TIME TEAM",font=HEAD,size=8.5,bold=True,color=MUT,tracking=1.5)
    core=[("t_akhtar.png","Akhtar Hussain","Mission Lead","IIT Delhi · ex-Bain · strategy, programmes, new initiatives, partnerships and product"),
          ("t_shubham.png","Shubham Kumar","Policy & Advocacy","NALSAR Hyderabad · programmes, policy advocacy, legal research, government relations"),
          ("t_muskan.png","Muskan Yadav","Programmes","TISS · district programmes, Project Rakshak, infrastructure interventions, DRSC work"),
          ("t_krishna.png","Krishna Jain","Tech & Product","Builds and runs Aasha, SATARK and the open public data platforms"),
          ("t_aastha.png","Aastha Shreeharsh","Development Communications","HKUST · institutional communications, government relations"),
          ("t_rohan.png","Rohan Sharma","Cause Marketing","Awareness campaigns, digital advocacy, community mobilisation"),
          ("t_ishita.png","Ishita Rathore","People & Culture","Talent and hiring, volunteers, the 75+ intern engineering corps"),
          ("t_yuvraj.png","Yuvraj Yadav","Operations","Field operations across 18 cities · works with police on SATARK and enforcement rollouts"),
    ...
    for j,(img,nm,role,cred) in enumerate(core):
        r,c=divmod(j,3)
        x=ML+c*cw_; y=I(3.54)+r*I(0.76)
        circp(s,img,x,y+I(0.04),I(0.58))
        text(s,x+I(0.72),y,I(3.28),I(0.3),[(nm+"  ",{'bold':True,'size':9.5}),(role,{'color':BRAND,'size':8,'bold':True})],size=9,ls=1.0)
        text(s,x+I(0.72),y+I(0.24),I(3.28),I(0.5),cred,size=7.5,color=MUT,ls=1.12)
    y3=I(3.54)+3*I(0.76)
    rect(s,ML+I(4.36),y3+I(0.02),I(5.6),I(0.6),fill=BR_T,rounded=True,radius=0.12)
    text(s,ML+I(4.58),y3+I(0.02),I(5.2),I(0.6),
         [("Plus 75+ engineering interns and 31 student audit teams.",{'bold':True,'color':BRAND})],size=9,ls=1.15,anchor=MSO_ANCHOR.MIDDLE)
    rect(s,ML,I(6.55),CW,I(0.42),fill=MIST,rounded=True,radius=0.14)
    text(s,ML+I(0.22),I(6.62),CW-I(0.45),I(0.32),
         [("In-house capabilities: ",{'bold':True}),
          ("road-engineering audits · legal & policy research · AI product development · district operations · behaviour-change communication",{'color':MUT})],size=9,ls=1.15)
    footer(s,wg,idx,total)
    return s
...
def bench(idx,total,wg):
    ...
    heads=[("midha","Justice J.R. Midha","Delhi High Court (Retd.)"),("damle","Abhay Damle","Ex-JS, MoRTH"),
           ...
           ("raman","Aishwarya Raman","OMI Foundation"),
...
def wg2_body(wg,total,start):
    ...
    cards=[("clipboard-list","Students audit","Teams from IITs, NITs and SPAs collect data on-site: traffic, conflicts, 900+ stakeholder surveys.",a5("guntur_junction.jpg"),0.4),
           ("badge-check","Experts verify","Mentors anchor every audit to IRC codes and MoRTH checklists before anything is proposed.",a6("ph_auth_files.jpeg"),0.3),
    ...
    stat(s,ML+I(7.1),I(2.3),I(3.4),"20–80%","crash reduction where low-cost engineering fixes are applied (iRAP & PIARC)",vsize=30,card=True,h=I(1.3),color=GREEN)
    text(s,ML,I(3.95),I(7.3),I(1.3),
         "Raised crossings, traffic calming, lighting and safer intersections are proven, affordable counter-measures. What limits progress is steady, supervised auditing capacity and follow-through, working under the road-owning authority.",
    ...
    photo(s,a6("ph_auth_desk.jpeg"),I(8.35),I(3.95),I(4.35),I(2.5),fill=True,anchor=0.35,
          caption="Audit findings on the executive engineer's desk, the road-owning authority acts.")
    footer(s,wg,i,total); i+=1
    # footprint
    s=slide()
    ...
    rows=[("book-check","IRC codes & MoRTH checklists","Every audit instrument is built on them, nothing bespoke, nothing incompatible.",BR_T,"#4A35FF"),
          ("link","Expert review at every step","Prof. Geetam Tiwari (TRIP Centre, IIT Delhi) chairs our advisory work; every instrument is expert-reviewed before field use.",GR_T,"#0A9955"),
    ...
    photo(s,a6("ph_auth_handover.jpeg"),I(8.5),I(5.1),I(4.2),I(1.55),fill=True,anchor=0.2,border=True)
    footer(s,wg,i,total); i+=1
    ...
    header(s,"04 · Vulnerable road users",
           ...
           sect=(4,"PROJECT RAKSHAK"))
    text(s,ML,I(2.32),I(7.6),I(0.26),"OUR PUBLISHED STUDY: 'WHEN RISKS BECOME ROUTINE' (FEB 2026)",font=HEAD,size=8.5,bold=True,color=BRAND,tracking=1.3)
    gt=[("~45%","of India's crash fatalities are two-wheeler users, rising year on year"),
        ("1 in 2","delivery riders in a 431-rider Mumbai crash study had already crashed"),
        ("~70%","of families at our crash-compensation helpdesks are vulnerable road users")]
    for j,(v,l) in enumerate(gt):
        x=ML+j*I(2.56)
        rect(s,x,I(2.68),I(2.44),I(1.14),fill=MIST,rounded=True,radius=0.10)
        text(s,x+I(0.15),I(2.78),I(2.2),I(0.4),v,font=HEAD,size=18,bold=True,color=BRAND)
        text(s,x+I(0.15),I(3.2),I(2.2),I(0.55),l,size=8,color=MUT,ls=1.15)
    text(s,ML,I(3.98),I(7.5),I(0.6),
         "Forty pages and 87 references, published February 2026. Enforcement sees the risk too: Bengaluru's challans against delivery riders rose from 30,968 in 2023 to 63,718 in nine months of 2025. The study lands on two first levers, rider training and helmet compliance, and has been shared with platforms and police.",
         size=9.5,color=INK,ls=1.3)
    rows=[("footprints","Pedestrian-first audits","Every Rakshak audit documents crossings, footpaths and lighting as designable protections, 900+ stakeholders consulted.",BR_T,"#4A35FF"),
    ...
    y=I(4.85)
    for j,(ic,t,d,tint,col) in enumerate(rows):
        irow(s,ML,y+j*I(0.74),I(7.5),ic,t,d,tint,col,tsize=11,dsize=9)
    photo_fit(s,a6("chart_vru_trends.png"),I(8.5),I(2.32),I(4.2),I(2.2),caption="Fatality trends by road-user type, 2014–2023 (MoRTH; via IndiaSpend).")
    photo(s,a5("ph_gig_riders.jpg"),I(8.5),I(5.15),I(4.2),I(1.5),fill=True,anchor=0.35,border=True)
    footer(s,wg,i,total); i+=1
    return i
...
def wg5_body(wg,total,start):
    ...
    for j,(v,l) in enumerate(tiles):
        x=ML+j*(tw+I(0.12)); y=I(2.3)
        rect(s,x,y,tw,I(1.2),fill=MIST,rounded=True,radius=0.08)
        text(s,x+I(0.18),y+I(0.1),tw-I(0.36),I(0.5),v,font=HEAD,size=20,bold=True,color=BRAND)
        text(s,x+I(0.18),y+I(0.6),tw-I(0.36),I(0.55),l,size=9,color=MUT,ls=1.2)
    yb=I(3.85)
    rect(s,ML,yb,CW,I(2.95),fill=PAPER,line=LINE,rounded=True,radius=0.06)
    photo_fit(s,a6("cov_deposited.png"),ML+I(0.2),yb+I(0.22),I(1.9),I(2.5),caption=None)
    text(s,ML+I(2.3),yb+I(0.18),I(9.5),I(0.32),"WHAT OUR RESEARCH FOUND: 'DEPOSITED BUT NOT DELIVERED'",font=HEAD,size=9,bold=True,color=BRAND,tracking=1.2)
    text(s,ML+I(2.3),yb+I(0.5),I(6.0),I(0.62),
         "Money that families have already won in court, deposited by insurers, sitting unclaimed. The Supreme Court took note in 2025 and ordered public dashboards and claimant tracing.",size=9.5,color=INK,ls=1.28)
    rows=[("bell-off","Families are never told","No proactive notification exists when an insurer deposits the award; many families learn months or years later, or never."),
          ("file-x","Withdrawal stalls on paperwork","A 15–20 day process in theory; in practice, documents, missing bank details in older judgments, and office hand-offs stall it."),
          ("scale","Advocate incentives cut against families","Consultations found contingency fees of 20–30%, and little reason for counsel to speed up the final hand-over.")]
    yr=yb+I(1.28)
    for j,(ic,t,d) in enumerate(rows):
        icon_disc(s,ic,ML+I(2.3),yr+j*I(0.56),I(0.36),AM_T,"#E58900")
        text(s,ML+I(2.78),yr+j*I(0.56)-I(0.01),I(2.55),I(0.5),t,font=HEAD,size=9,bold=True,color=INK,ls=1.05)
        text(s,ML+I(5.42),yr+j*I(0.56)-I(0.01),I(3.35),I(0.52),d,size=8,color=MUT,ls=1.15)
    tx=ML+I(9.0)
    text(s,tx,yb+I(0.5),I(2.9),I(0.26),"UNCLAIMED, BY HIGH COURT",font=HEAD,size=8,bold=True,color=MUT,tracking=1.2)
    table=[("Bombay","₹459 Cr"),("Gujarat","₹282 Cr"),("Allahabad","₹239 Cr"),("Goa","₹3.6 Cr"),("Calcutta","₹2.5 Cr")]
    yt=yb+I(0.82)
    for j,(st,amt) in enumerate(table):
        text(s,tx,yt+j*I(0.30),I(1.6),I(0.26),st,size=8.5,color=INK,ls=1.0)
        text(s,tx+I(1.65),yt+j*I(0.30),I(1.2),I(0.26),amt,font=HEAD,size=8.5,bold=True,color=BRAND,align=PP_ALIGN.RIGHT,ls=1.0)
    hline(s,tx,yt+I(1.54),I(2.85))
    text(s,tx,yt+I(1.62),I(1.6),I(0.26),"Five HCs only",size=8.5,color=MUT,ls=1.0)
    text(s,tx+I(1.65),yt+I(1.62),I(1.2),I(0.26),"~₹986 Cr",font=HEAD,size=9.5,bold=True,color=AMBER,align=PP_ALIGN.RIGHT,ls=1.0)
    text(s,ML,I(6.55),CW,I(0.35),"Source: Supreme Court, In Re: Compensation Amounts Deposited with MACTs and Labour Courts (2025); CFI stakeholder consultations, Oct–Nov 2025. The national figure is likely far higher.",size=8.5,color=MUT,ls=1.2)
    footer(s,wg,i,total); i+=1
    # justice unserved
    s=slide()
    ...
    rect(s,I(9.05),I(2.15),I(3.65),I(4.05),fill=MIST,rounded=True,radius=0.08)
    text(s,I(9.25),I(2.3),I(3.3),I(0.3),"THE CLAIM RATE, NATIONALLY",font=HEAD,size=8.5,bold=True,color=MUT,tracking=1.2)
    bars=[("68,584","hit-and-run crashes recorded in 2024",I(3.1),INK),
          ("~30,000","of them kill, every year (MoRTH)",I(1.36),RGBColor(0x8A,0x8F,0x98)),
          ("9,813","claims registered, FY 2025-26",I(0.45),BRAND),
          ("7,348","claims paid, to 28 Feb 2026",I(0.34),GREEN)]
    yb=I(2.72)
    for v,l,w,col in bars:
        rect(s,I(9.25),yb,w,I(0.28),fill=col,rounded=True,radius=0.5)
        text(s,I(9.25),yb+I(0.3),I(3.3),I(0.28),[(v+"  ",{'bold':True,'size':10,'color':col}),(l,{'size':8,'color':MUT})],ls=1.0)
        yb=yb+I(0.72)
    text(s,I(9.25),I(5.62),I(3.25),I(0.5),"About 1 claim for every 7 crashes, up 48x since FY23 (205), and still far from the loss. Source: Lok Sabha reply AU6419, April 2026.",size=8.5,color=MUT,ls=1.2,italic=True)
    text(s,ML,I(6.3),I(8.2),I(0.5),
    ...
    photo(s,a5("ph_helpdesk_camp.jpg"),I(8.5),I(2.2),I(4.2),I(2.2),fill=True,anchor=0.55,
          caption="Legal-rights helpdesk with district administration, Jan 2026.")
    photo(s,a5("ph_law_students.jpg"),I(8.5),I(4.95),I(4.2),I(1.6),fill=True,anchor=0.42,
    ...
    rows=[("file-warning","What holds claims back","11–13 documents demanded vs the 7 court-agreed standard · ~50% of files lack a usable family phone number · manual hand-offs between six offices.",AM_T,"#E58900"),
          ...
          ("route","Office-by-office mapping","We walked the file's actual path, police, SDM office, insurer desk, treasury, and mapped every hand-off where families drop out.",BR_T,"#4A35FF"),
    ...
    header(s,"04 · Post-crash care, the pipeline, measured",
           ...
           sect=(4,"POST-CRASH CARE"))
    tiles=[("921 of 1,026","hit-and-run claims pending nationally were stalled at documentation alone (RTI, July 2024)","A single national document checklist"),
           ("11–13 vs 7","documents demanded of claimants in practice, against the court-agreed standard of seven","Enforce the seven-document standard"),
           ("~50%","of case files lack a usable family phone number","A traceability norm at first contact"),
           ("~60 days","typical intake delay where claims surface only through bi-monthly committee meetings","A time-bound intake standard"),
           ("1 in 7","hit-and-run crashes becomes a claim: 9,813 claims against 68,584 crashes in 2024","Awareness and intake norms at scale"),
           ("20–40","office visits for a family to keep a single claim alive today","A single-window process for families")]
    tw=I(3.95)
    for j,(v,l,lever) in enumerate(tiles):
        r,c=divmod(j,3)
        x=ML+c*(tw+I(0.12)); y=I(2.35)+r*I(1.62)
        rect(s,x,y,tw,I(1.48),fill=MIST,rounded=True,radius=0.08)
        text(s,x+I(0.18),y+I(0.1),tw-I(0.36),I(0.45),v,font=HEAD,size=19,bold=True,color=BRAND)
        text(s,x+I(0.18),y+I(0.56),tw-I(0.36),I(0.52),l,size=8.5,color=MUT,ls=1.15)
        text(s,x+I(0.18),y+I(1.13),tw-I(0.36),I(0.3),[("The standard: ",{'bold':True,'size':8,'color':GREEN}),(lever,{'size':8,'color':GREEN})],ls=1.1)
    text(s,ML,I(5.85),CW,I(0.8),
         [("The claim's other side: ",{'bold':True}),
          ("guidance from Justice J.R. Midha (Retd.), architect of the FaSTDAR process · consultations with insurers on settlement practice · working partners across MACT courts, DLSAs and legal-services clinics, Mishika Singh (Neev Foundation), Yash Agarwal (Public Policy India), Sanchit Seth (Advocate, Delhi MACT).",{'color':MUT})],size=10,ls=1.35)
    footer(s,wg,i,total); i+=1
    return i

# ================================================================ WG-3 BODY
def wg3_body(wg,total,start):
    i=start
    # the problem
    s=slide()
    header(s,"04 · Gig & two-wheeler safety, the problem",
           [("The fastest-growing risk on Indian roads ",False),("rides a two-wheeler",True)],
           lead="Gig and delivery work concentrates the exposure: entire working days on the road, on vehicles and licences that see comparatively little scrutiny.",
           sect=(4,"GIG & TWO-WHEELER SAFETY"))
    tiles=[("~45%","of India's crash fatalities are two-wheeler users, and the share rises year on year"),
           ("23.5 M","gig and platform workers projected by 2029-30 (NITI Aayog)"),
           ("1 in 2","delivery riders in a 431-rider Mumbai crash study had already been in a crash")]
    tw=I(2.6)
    for j,(v,l) in enumerate(tiles):
        x=ML+j*(tw+I(0.12)); y=I(2.35)
        rect(s,x,y,tw,I(1.6),fill=MIST,rounded=True,radius=0.08)
        text(s,x+I(0.16),y+I(0.12),tw-I(0.32),I(0.45),v,font=HEAD,size=20,bold=True,color=BRAND)
        text(s,x+I(0.16),y+I(0.62),tw-I(0.32),I(0.9),l,size=8.5,color=MUT,ls=1.2)
    text(s,ML,I(4.25),I(8.0),I(1.1),
         "This is also where our road safety and compensation work meet: many gig riders injured on the job are unaware of the compensation they are entitled to. Roughly 70% of the families at our crash-compensation helpdesks are vulnerable road users. Our NextMile ideathon saw student teams working on delivery-rider safety and on compensation reform arrive, independently, at the same population.",
         size=10.5,color=INK,ls=1.35)
    rect(s,ML,I(5.6),I(8.0),I(1.05),fill=AM_T,rounded=True,radius=0.10)
    text(s,ML+I(0.22),I(5.72),I(7.6),I(0.85),
         [("Enforcement sees the pressure too: ",{'bold':True,'color':AMBER}),
          ("Bengaluru's challans against delivery riders rose from 30,968 in 2023 to 63,718 in the first nine months of 2025.",{'color':AMBER})],size=10.5,ls=1.3)
    photo(s,a5("ph_gig_riders.jpg"),I(8.95),I(2.35),I(3.75),I(3.5),fill=True,anchor=0.35,
          caption="Delivery-rider outreach with Swiggy and Instamart riders, Road Safety Month 2026.")
    footer(s,wg,i,total); i+=1
    # published study
    s=slide()
    header(s,"04 · Gig & two-wheeler safety, our research",
           [("'When Risks Become Routine', ",False),("published February 2026",True)],
           lead="A study of the road risk produced by delivery and ride-hailing work: 40 pages, 87 references, fieldwork on the road with riders.",
           sect=(4,"GIG & TWO-WHEELER SAFETY"))
    rows=[("banknote","Structural & occupational conditions","Pay-per-piece incentive design and delivery timelines create sustained time pressure over long shifts.",AM_T,"#E58900"),
          ("hard-hat","Operational & behavioural risk","Helmet non-compliance, largely self-taught rider training, and phone use while riding under app-driven time pressure.",RD_T,"#E11900"),
          ("git-compare","Data fragmentation","Crash and near-miss data sit separately with platforms, insurers and police; no one holds a shared view of rider-level risk.",BR_T,"#4A35FF"),
          ("zap","A regulatory blind spot","Low-speed e-bikes under 250W / 25 km/h require no licence or registration, leaving a growing share of last-mile fleets outside routine enforcement.",TE_T,"#0E8A8A"),
          ("map-pin","Infrastructure & site design","The layout of dark stores and delivery hubs can itself push riders into unsafe manoeuvres, through poorly designed access roads and pick-up zones.",GR_T,"#0A9955")]
    y=I(2.3)
    for j,(ic,t,d,tint,col) in enumerate(rows):
        irow(s,ML,y+j*I(0.82),I(7.7),ic,t,d,tint,col,tsize=11,dsize=9)
    x3=I(8.75)
    rect(s,x3,I(2.3),I(3.95),I(2.1),fill=MIST,rounded=True,radius=0.08)
    text(s,x3+I(0.2),I(2.45),I(3.55),I(0.26),"FIELDWORK, NOT ONLY DESKWORK",font=HEAD,size=8.5,bold=True,color=BRAND,tracking=1.2)
    text(s,x3+I(0.2),I(2.76),I(3.55),I(1.5),
         "The study included shadowing a delivery rider through a full nine-hour shift, documented on video, to capture pacing, routing and time pressure directly.",size=9.5,color=INK,ls=1.3)
    rect(s,x3,I(4.6),I(3.95),I(1.95),fill=PAPER,line=LINE,rounded=True,radius=0.08)
    text(s,x3+I(0.2),I(4.75),I(3.55),I(0.26),"INTERNATIONAL PRECEDENT",font=HEAD,size=8.5,bold=True,color=MUT,tracking=1.2)
    text(s,x3+I(0.2),I(5.06),I(3.55),I(1.4),
         "Spain's 2021 Riders' Law brought algorithmic transparency to dispatch systems; Australia maps delivery-rider risk as a system. India's conversation is only beginning.",size=9.5,color=MUT,ls=1.3)
    footer(s,wg,i,total); i+=1
    # 300-rider study
    s=slide()
    header(s,"04 · Gig & two-wheeler safety, the next step",
           [("A 300-rider study is now under way, ",False),("to quantify what we surfaced",True)],
           lead="The published brief gives a clear qualitative picture. The study now in the field sizes it, by hearing directly from riders at scale.",
           sect=(4,"GIG & TWO-WHEELER SAFETY"))
    cards=[("gauge","How often","How frequently riders actually experience near-misses and crashes, measured across a large sample rather than a handful of conversations.",BR_T,"#4A35FF"),
           ("list-checks","Which factors","Which risk factors matter most in practice: fatigue, incentive design, training gaps, so interventions start where the risk is.",AM_T,"#E58900"),
           ("users","Which riders","What distinct rider profiles exist, so that training, equipment and platform changes can be targeted rather than generic.",TE_T,"#0E8A8A")]
    tw=I(3.95)
    for j,(ic,t,d,tint,col) in enumerate(cards):
        x=ML+j*(tw+I(0.12)); y=I(2.4)
        rect(s,x,y,tw,I(2.5),fill=PAPER,line=LINE,rounded=True,radius=0.08)
        icon_disc(s,ic,x+I(0.2),y+I(0.22),I(0.52),tint,col)
        text(s,x+I(0.2),y+I(0.92),tw-I(0.4),I(0.35),t,font=HEAD,size=13,bold=True,color=INK)
        text(s,x+I(0.2),y+I(1.32),tw-I(0.4),I(1.05),d,size=9.5,color=MUT,ls=1.28)
    chip(s,ML,I(5.25),I(2.6),I(0.44),"IN THE FIELD NOW · 2026",fill=GR_T,color=GREEN,size=9.5)
    text(s,ML,I(5.95),CW,I(0.8),
         [("Reviewed by the people who know the terrain: ",{'bold':True}),
          ("research design reviewed by Dr. Richa Ahuja (IIT Kharagpur) · site-design dimension shaped with Dr. Deepty Jain and Prof. Geetam Tiwari (TRIP Centre, IIT Delhi) · enforcement realities from former traffic-police leadership of Delhi, Gurugram and Bengaluru.",{'color':MUT})],size=10,ls=1.3)
    footer(s,wg,i,total); i+=1
    # aggregator early look
    s=slide()
    header(s,"04 · Gig & two-wheeler safety, aggregator vehicles",
           [("Vehicle compliance among aggregators, ",False),("an early look",True)],
           lead="A February 2025 review of traffic challans against ride-hailing vehicles across four platforms, exploratory, and revealing.",
           sect=(4,"GIG & TWO-WHEELER SAFETY"))
    stat(s,ML,I(2.35),I(3.4),"56.8%","of ride-hailing vehicles reviewed carried more than three recorded challans",vsize=30,card=True,h=I(1.35))
    stat(s,ML+I(3.55),I(2.35),I(3.4),"40+","challans on a single vehicle, the outlier a per-vehicle view surfaces and an aggregate statistic hides",vsize=30,card=True,h=I(1.35),color=AMBER)
    stat(s,ML+I(7.1),I(2.35),I(3.4),"2020","the Aggregator Guidelines first required e-challan clearance records; the 2025 revision makes clearance a precondition",vsize=30,card=True,h=I(1.35),color=TEAL)
    text(s,ML,I(4.05),I(11.9),I(0.85),
         "The rule already exists. A vehicle still carrying forty-plus outstanding challans, found well after the requirement took effect, points to a gap in verification and enforcement rather than in the rule itself. That is a technology and process problem, and it is one we have worked on before.",
         size=11,color=INK,ls=1.35)
    rect(s,ML,I(5.15),CW,I(1.15),fill=BR_T,rounded=True,radius=0.10)
    icon_disc(s,"cpu",ML+I(0.22),I(5.35),I(0.5),PAPER,"#4A35FF")
    text(s,ML+I(0.92),I(5.3),I(11.2),I(0.9),
         [("SATARK shows per-vehicle verification working at scale: ",{'bold':True,'color':BRAND}),
          ("over 3,00,000 vehicles screened with police in Jaipur, Bengaluru and Gurugram, surfacing exactly the repeat-offender vehicles an onboarding check needs to catch.",{'color':BRAND})],size=10.5,ls=1.3)
    footer(s,wg,i,total); i+=1
    # vehicle standards sessions
    s=slide()
    header(s,"04 · Vehicle standards, our engagement",
           [("Working sessions with the people who ",False),("wrote the standards",True)],
           lead="Two expert sessions pair the authors and auditors of India's vehicle-safety standards with newer technology and operational practice.",
           sect=(4,"GIG & TWO-WHEELER SAFETY"))
    cards=[("bike","Two-wheeler safety technology","ADAS for two-wheelers, and the case for a national two-wheeler safety stack.",
            "Balraj Bhanot, former Director ARAI, former Chairman CMVR-TSC & BIS TED-28, with Aayush Kumar, COO & co-founder, BYTES (acquired by Cautio).",BR_T,"#4A35FF"),
           ("bus","Bus safety","Fire detection and suppression systems, and the gap between code and ground-level compliance.",
            "Dattatraya Saste, former Technical Consultant MoRTH and former Convener, Bus Code & Safety Regulations, with Shekhar Dhole, Technical Secretariat in-charge, CIRT; Panel Convenor, AIS-052/119.",TE_T,"#0E8A8A")]
    tw=I(5.98)
    for j,(ic,t,d,who,tint,col) in enumerate(cards):
        x=ML+j*(tw+I(0.13)); y=I(2.35)
        rect(s,x,y,tw,I(2.75),fill=PAPER,line=LINE,rounded=True,radius=0.08)
        icon_disc(s,ic,x+I(0.22),y+I(0.24),I(0.52),tint,col)
        text(s,x+I(0.9),y+I(0.32),tw-I(1.1),I(0.35),t,font=HEAD,size=13.5,bold=True,color=INK)
        text(s,x+I(0.22),y+I(0.95),tw-I(0.44),I(0.6),d,size=10,color=INK,ls=1.28)
        hline(s,x+I(0.22),y+I(1.68),tw-I(0.44))
        text(s,x+I(0.22),y+I(1.8),tw-I(0.44),I(0.85),who,size=9,color=MUT,ls=1.25)
    rect(s,ML,I(5.45),CW,I(0.95),fill=GR_T,rounded=True,radius=0.10)
    text(s,ML+I(0.22),I(5.57),CW-I(0.45),I(0.75),
         [("These sessions were a first step, and the relationships hold: ",{'bold':True,'color':GREEN}),
          ("with ARAI, CMVR-TSC and CIRT veterans, and with two-wheeler ADAS developers, all available to this Working Group.",{'color':GREEN})],size=10.5,ls=1.3)
    footer(s,wg,i,total); i+=1
    # the missing table
    s=slide()
    header(s,"04 · Gig & two-wheeler safety, the conversation ahead",
           [("A safety conversation on gig work, ",False),("ready to begin",True)],
           lead="Gig-work policy so far has been framed around social security and labour conditions. This Working Group can convene its road safety dimension.",
           sect=(4,"GIG & TWO-WHEELER SAFETY"))
    rows=[("landmark","What we heard from the Ministry","Our consultation with the Ministry of Labour and Employment confirmed the framing: platform-work policy centres on welfare boards, insurance and social security.",BR_T,"#4A35FF"),
          ("globe","What is happening elsewhere","The road safety dimension of gig work has begun receiving dedicated attention in Vietnam and Taiwan; a comparable conversation in India is yet to take shape.",TE_T,"#0E8A8A"),
          ("users-round","Who needs to sit together","Platforms, police, insurers, regulators and researchers each hold a fragment of rider risk. No one holds the shared view; a Working Group table can.",GR_T,"#0A9955")]
    y=I(2.45)
    for j,(ic,t,d,tint,col) in enumerate(rows):
        irow(s,ML,y+j*I(1.0),I(12.0),ic,t,d,tint,col)
    text(s,ML,I(5.6),I(12),I(0.26),"ALREADY CONSULTED FOR THIS WORK",font=HEAD,size=8.5,bold=True,color=MUT,tracking=1.4)
    who=["Ministry of Labour & Employment","V.V. Giri National Labour Institute","Traffic police leadership: Delhi · Gurugram · Bengaluru","Ex-operations, Swiggy & Zomato","Fairwork India researchers","TRIP Centre, IIT Delhi · IIT Kharagpur"]
    for j,w in enumerate(who):
        c=j%3; r=j//3
        chip(s,ML+c*I(4.06),I(5.92)+r*I(0.5),I(3.94),I(0.42),w,fill=BR_T,color=BRAND,size=8.5)
    footer(s,wg,i,total); i+=1
    # nextmile pipeline
    s=slide()
    header(s,"04 · Gig & two-wheeler safety, the talent pipeline",
           [("Young policy talent is already ",False),("working this problem",True)],
           lead="Delivery-rider safety was the most-discussed track of our NextMile policy ideathon, and it feeds a standing dialogue with experts.",
           sect=(4,"GIG & TWO-WHEELER SAFETY"))
    photo(s,a5("ph_ideathon_winners.jpg"),ML,I(2.3),I(4.4),I(2.7),fill=True,anchor=0.2,
          caption="NextMile grand finale, New Delhi: the delivery-rider safety track produced three finalist teams.")
    rows=[("route","Rider pods for fatigue management","Team Strategists (IIM Bangalore) proposed pod-based fatigue management, drawing on a Chennai model.",BR_T,"#4A35FF"),
          ("timer","The SafeMile Protocol","Team A² (Kolkata) proposed aligning delivery ETAs to safe-speed benchmarks, with tiered delivery windows and digital rest-break enforcement.",TE_T,"#0E8A8A"),
          ("mic","A standing dialogue","Our National Road Safety Dialogue Series has run a dedicated session on delivery-rider safety, with platforms and researchers in the room.",AM_T,"#E58900")]
    x3=I(5.35)
    y=I(2.3)
    for j,(ic,t,d,tint,col) in enumerate(rows):
        icon_disc(s,ic,x3,y+j*I(1.06),I(0.42),tint,col)
        text(s,x3+I(0.58),y+j*I(1.06)-I(0.03),I(2.6),I(0.6),t,font=HEAD,size=11,bold=True,color=INK,ls=1.1)
        text(s,x3+I(3.3),y+j*I(1.06)-I(0.03),I(4.0),I(0.95),d,size=9.5,color=MUT,ls=1.22)
    rect(s,ML,I(5.75),CW,I(0.8),fill=MIST,rounded=True,radius=0.12)
    text(s,ML+I(0.22),I(5.87),CW-I(0.45),I(0.6),
         [("Our stated priority from that work: ",{'bold':True}),
          ("rider safety taken forward with governments and platforms together, small pilots first, progress reported transparently.",{'color':MUT})],size=10,ls=1.25)
    footer(s,wg,i,total); i+=1
    # rare combination
    s=slide()
    header(s,"04 · What this adds up to",
           [("A combination that is ",False),("rare to find together",True)],
           lead="Few organisations working on vehicle standards, fleets or aggregator regulation sit inside both the industry and the research worlds at once.",
           sect=(4,"GIG & TWO-WHEELER SAFETY"))
    rows=[("link","Technical access on request","Standing relationships with the people who have written and audited India's vehicle-safety standards, from ARAI and the CMVR Technical Standing Committee to CIRT's bus-code veterans and two-wheeler ADAS developers.",BR_T,"#4A35FF"),
          ("building-2","An industry vantage point","Visibility into vehicle markets, fleet operations and used-vehicle certification that comes from our founding by Cars24, alongside the research and policy work.",AM_T,"#E58900"),
          ("file-search","A two-wheeler and last-mile evidence base","A growing, increasingly quantified body of research on the two-wheeler and gig-rider segment specifically, the fastest-growing source of road deaths in the country.",TE_T,"#0E8A8A"),
          ("badge-check","A safety-specific view on aggregators","Early evidence on vehicle-compliance verification gaps, and a perspective on aggregator accountability that sits alongside, rather than duplicates, the social-security lens.",GR_T,"#0A9955")]
    y=I(2.35)
    for j,(ic,t,d,tint,col) in enumerate(rows):
        irow(s,ML,y+j*I(1.05),I(12.0),ic,t,d,tint,col)
    footer(s,wg,i,total); i+=1
    return i
...
def build_wg2():
    ...
    letter(2,
      "In our first year we audited 31 high-risk locations across 18 cities with teams from 20 institutions, secured 25+ written approvals from road-owning authorities, and implementation has started at 15 sites, with 7 already fully built. SATARK, our AI enforcement platform, has screened over 3,00,000 vehicles for police in Jaipur and Bengaluru and has now reached Gurugram. Four open public data platforms and a national youth pipeline complete the picture.",
      2,TOTAL)
    ...
    why_fit(2,6,TOTAL,
        focus=[("construction","Black-spot rectification at scale","Our audit-to-implementation pipeline is running in 18 cities, the Group's rectification agenda is our core work.",BR_T,"#4A35FF"),
               ("gauge","Standards for audits & speed management","Audit instruments anchored to IRC codes and MoRTH checklists, with masterclass-trained student teams.",GR_T,"#0A9955"),
               ("footprints","Protecting vulnerable road users","Pedestrian-first audits, our published gig-rider safety study, school-zone screening, evidence ready for VRU standards.",AM_T,"#E58900")],
        bring=["31 trained audit teams","Public tracking portal","District pilots: GGM · SWD · Jaipur","Gig-rider safety study","7 sites fully built","Expert bench on call"],
        smes=[("Dr. Richa Ahuja","Asst Professor, IIT Kharagpur","x_richa.png",True),
              ("Prof. Geetam Tiwari","TRIP Centre, IIT Delhi","x_gtiwari.png",True),
              ("Abhay Damle","Ex-Joint Secretary, MoRTH","x_damle.png",True),
              ("Dipak Dash","Senior journalist, Times of India","x_dash.png",True),
              ("Sarika Panda Bhatt","Co-founder & Trustee, Raahgiri Foundation","x_sarika.png",True)])
    portfolio(7,TOTAL,2)
    satark(8,TOTAL,2)
    opendata(9,TOTAL,2)
    ...
    close(2,28,TOTAL)
    save(os.path.join(BUILD,"CFI_NRSB_WG2_v7.pptx"))
...
def build_wg5():
    ...
    why_fit(5,6,TOTAL,
        ...
        smes=[("Justice J.R. Midha","Retd., Delhi HC · MACT process architect","x_pp_midha.png",True),
              ("Piyush Tewari","SaveLIFE Foundation · Good Samaritan Law","x_tewari.png",True),
    ...
    close(5,26,TOTAL)
    save(os.path.join(BUILD,"CFI_NRSB_WG5_v7.pptx"))

def build_wg3():
    new_deck(); TOTAL=26
    cover(3,"Safe Vehicles, Commercial Transport & Future Mobility",
          ["Published research on gig and two-wheeler rider risk, a 300-rider study under way",
           "Vehicle standards work with the people who wrote them: ARAI, CMVR-TSC, CIRT",
           "Compliance technology proven with police: 3,00,000+ vehicles screened by SATARK"],1,TOTAL,
          icons=("bike","file-search","cpu"))
    letter(3,
      "Two-wheeler riders are the largest and fastest-growing category of road deaths, and gig and delivery riders spend entire working days inside that risk. We published 'When Risks Become Routine', our study of gig-rider road risk, in February 2026, and a 300-rider study is now in the field to quantify what it surfaced. Expert sessions on two-wheeler ADAS and bus fire safety have brought veterans of ARAI, CMVR-TSC and CIRT to our table, and an early review of ride-hailing vehicles found 56.8% carrying more than three recorded challans. SATARK, our vehicle-screening platform with police in three cities, shows compliance technology working at scale.",
      2,TOTAL)
    agenda(3,"Gig & two-wheeler safety in depth","17 – 24","25 – 26",3,TOTAL)
    about(4,TOTAL,3)
    approach(5,TOTAL,3)
    why_fit(3,6,TOTAL,
        focus=[("bike","Two-wheeler & rider safety evidence","Two-wheeler riders carry the fastest-growing share of road deaths; our published study and 300-rider fieldwork sit exactly here.",AM_T,"#E58900"),
               ("bus","Vehicle standards & commercial fleets","Expert sessions on two-wheeler ADAS and bus fire safety, with the veterans of ARAI, CMVR-TSC and CIRT, and an industry vantage point through Cars24.",BR_T,"#4A35FF"),
               ("cpu","Compliance technology for fleets","SATARK screens vehicles at scale with police; our aggregator review shows exactly where challan verification breaks down.",TE_T,"#0E8A8A")],
        bring=["Published gig-rider study","300-rider study in the field","ARAI · CMVR-TSC · CIRT bench","SATARK live in 3 cities","Aggregator challan evidence","Platform & police relationships"],
        smes=[("Balraj Bhanot","Ex-Chairman, CMVR-TSC · ex-ARAI","BB",False),
              ("Dattatraya Saste","Ex-MoRTH · Bus Code Convener","DS",False),
              ("Prof. Geetam Tiwari","TRIP Centre, IIT Delhi","x_gtiwari.png",True),
              ("Dr. Richa Ahuja","Asst Professor, IIT Kharagpur","x_richa.png",True),
              ("Akhilesh Srivastava","ITS India · IRF","x_asrivastava.png",True)])
    portfolio(7,TOTAL,3)
    satark(8,TOTAL,3)
    opendata(9,TOTAL,3)
    victim_suite(10,TOTAL,3)
    youth(11,TOTAL,3)
    ground(12,TOTAL,3)
    dhoni(13,TOTAL,3)
    partners(14,TOTAL,3)
    team(15,TOTAL,3)
    bench(16,TOTAL,3)
    nxt=wg3_body(3,TOTAL,17)
    contribution(3,[("A rider-risk evidence base for standards","The published study, the 300-rider quantification now in the field, and rider profiles that let two-wheeler, fleet and aggregator standards start from measured risk."),
                    ("Technical access to standards veterans","Standing relationships with ARAI, CMVR-TSC and CIRT veterans and two-wheeler ADAS developers, available for drafting, review and technical input."),
                    ("A safety-specific view on aggregators","Early vehicle-compliance evidence, and working relationships with platforms, police and labour institutions, for an aggregator conversation framed on road safety."),
                    ("Compliance technology, proven with police","SATARK's per-vehicle screening experience, offered as working input to verification and enforcement design for commercial and aggregator fleets.")],nxt,TOTAL)
    close(3,26,TOTAL)
    save(os.path.join(BUILD,"CFI_NRSB_WG3_v1.pptx"))
