...
# ---------------------------------------------------------------- S1 COVER
def cover(wg,name,chips_,idx,total):
    s=slide()
    ...
    text(s,ML,I(1.5),I(8.2),I(0.3),"EXPRESSION OF INTEREST  ·  NATIONAL ROAD SAFETY BOARD",font=HEAD,size=10.5,bold=True,color=BRAND,tracking=2.2)
    text(s,ML,I(1.92),I(8.3),I(0.85),"Crashfree India",font=HEAD,size=43,bold=True,color=INK,ls=1.0)
    text(s,ML,I(2.82),I(8.3),I(0.85),f"Application to Working Group {wg}:\n{name}",font=HEAD,size=16.5,bold=True,color=BRAND,ls=1.25)
    text(s,ML,I(3.85),I(8.1),I(0.65),"A technology-led road safety nonprofit — backed by Cars24, operated by the Vision Zero\nTrust — working with public authorities across 18 cities.",size=11.5,color=MUT,ls=1.35)
    cw=I(2.62); gap=I(0.14); y=I(4.72)
    for i,(v,l) in enumerate(chips_):
        x=ML+i*(cw+gap)
        rect(s,x,y,cw,I(0.92),fill=MIST,rounded=True,radius=0.14)
        text(s,x+I(0.16),y+I(0.10),cw-I(0.3),I(0.42),v,font=HEAD,size=15,bold=True,color=BRAND,ls=1.0)
        text(s,x+I(0.16),y+I(0.47),cw-I(0.3),I(0.42),l,size=8.5,color=MUT,ls=1.1)
...
def why_fit(wg,idx,total,focus,bring,smes,photo_card=None):
    ...
    for nm,role,img,isphoto in smes:
        rect(s,x,I(5.45),I(2.97),I(1.15),fill=MIST,rounded=True,radius=0.10)
        if isphoto:
            photo(s,img,x+I(0.12),I(5.58),I(0.85),I(0.9),fill=True,border=False,anchor=0.12)
        else:
            circle(s,x+I(0.12),I(5.58),I(0.85),BR_T)
            text(s,x+I(0.12),I(5.56),I(0.85),I(0.9),img,font=HEAD,size=14,bold=True,color=BRAND,align=PP_ALIGN.CENTER,anchor=MSO_ANCHOR.MIDDLE)
        text(s,x+I(1.08),I(5.56),I(1.85),I(0.4),nm,font=HEAD,size=9.5,bold=True,color=INK,ls=1.05)
        text(s,x+I(1.08),I(5.95),I(1.85),I(0.6),role,size=8,color=MUT,ls=1.15)
        x=x+I(3.06)
    footer(s,wg,idx,total)
    return s
...
def opendata(idx,total,wg):
    ...
    header(s,"02 · What we have built — open public data",
           [("Four live data platforms — ",False),("free for any authority",True)],
           lead="Each one turns a scattered public record into structured, citable evidence. Self-updating; no login, no fee.",
           sect=(2,"WHAT WE HAVE BUILT"))
    cards=[("card_defect.png","Road-Infrastructure Defect Repository — 146 districts · 13 languages · refreshed daily"),
           ("card_parliament.png","Road Safety in Parliament — 1,423 questions since 2014 · 282 sitting MPs have never asked one"),
           ("card_sctracker.png","Supreme Court Litigation Tracker — order-by-order timelines of systemic cases"),
           ("card_crashdash.png","Crash Data Dashboard — MoRTH annuals as a usable product · 485 lives lost every day")]
    cw_=I(2.95); chh=I(2.18)
    for j,(f,cap) in enumerate(cards):
        r,c=divmod(j,2)
        x=ML+c*(cw_+I(0.14)); y=I(2.12)+r*I(2.42)
        photo(s,a6(f),x,y,cw_,chh,fill=True,anchor=0.0)
    text(s,ML,I(6.72),I(6.1),I(0.3),"Product cards from our internal data-stack brief, July 2026 — descriptions as shipped.",size=8.5,color=MUT)
    xr=I(6.9); wr=I(5.8)
    photo(s,a5("sctracker.png"),xr,I(2.12),wr,I(2.75),fill=True,anchor=0.0,
          caption="Live view, 1 Aug 2026 — 20 systemic cases · 191 orders indexed · 30 precedent judgments. Refreshes every Tuesday.")
    photo(s,a5("suraksha_cards.png"),xr,I(5.42),wr,I(1.02),fill=True)
    text(s,xr,I(6.5),wr,I(0.3),"Plus: Suraksha Mitra & Sadak Suraksha Suchak — citizen hazard reporting in Hindi (50+ hazards in the first 20 days).",size=8.5,color=MUT,ls=1.15)
    footer(s,wg,idx,total)
    return s
...
def youth(idx,total,wg):
    ...
    photo(s,a5("ph_vision_wall.jpg"),I(4.85),I(2.12),I(3.7),I(2.5),fill=True,anchor=0.3,
...
def dhoni(idx,total,wg):
    ...
    header(s,"02 · What we have built — public voice",
           [("MS Dhoni carries the message; ",False),("the press covers the work",True)],
           sect=(2,"WHAT WE HAVE BUILT"))
    ...
    text(s,I(7.4),I(1.95),I(5.3),I(0.26),"IN THE NATIONAL PRESS",font=HEAD,size=8.5,bold=True,color=MUT,tracking=1.6)
...
def team(idx,total,wg):
    ...
    header(s,"03 · Our people",
           ...
           sect=(3,"OUR PEOPLE"))
    trustees=[("AS","Amar Srivastava","President · Founder, Indian Road Safety Campaign · Member, National Road Safety Council"),
              ("DG","Deepanshu Gupta","Managing Trustee · Founder, IRSC · Global Youth Coalition for Road Safety"),
              ("GJ","Gajendra Jangid","General Secretary · Co-founder, Cars24")]
    x=ML
    for ini,nm,cred in trustees:
        rect(s,x,I(2.1),I(3.97),I(0.98),fill=MIST,rounded=True,radius=0.10)
        circle(s,x+I(0.13),I(2.23),I(0.72),BR_T)
        text(s,x+I(0.13),I(2.21),I(0.72),I(0.72),ini,font=HEAD,size=13,bold=True,color=BRAND,align=PP_ALIGN.CENTER,anchor=MSO_ANCHOR.MIDDLE)
        ...
        x=x+I(4.06)
    text(s,ML,I(3.35),I(12),I(0.26),"THE FULL-TIME TEAM",font=HEAD,size=8.5,bold=True,color=MUT,tracking=1.5)
    core=[("AH","Akhtar Hussain","Mission Lead","IIT Delhi"),
          ("SK","Shubham Kumar","Policy & Advocacy","NALSAR Hyderabad"),
          ("MY","Muskan Yadav","Programmes","TISS"),
          ("AS","Aastha Shreeharsh","Strategy & Ops","HKUST"),
          ("RS","Rohan Sharma","Cause Marketing","—"),
          ("IR","Ishita Rathore","People & Culture","—"),
          ("YY","Yuvraj Yadav","Product Design","—"),
          ("SK","Shipra Kushwaha","Graphic Design","—")]
    cw_=I(3.0)
    for j,(ini,nm,role,alma) in enumerate(core):
        r,c=divmod(j,4)
        x=ML+c*(cw_+I(0.03)); y=I(3.68)+r*I(1.06)
        rect(s,x,y,I(2.93),I(0.94),fill=PAPER,line=LINE,rounded=True,radius=0.10)
        circle(s,x+I(0.12),y+I(0.17),I(0.6),BR_T)
        text(s,x+I(0.12),y+I(0.15),I(0.6),I(0.6),ini,font=HEAD,size=11,bold=True,color=BRAND,align=PP_ALIGN.CENTER,anchor=MSO_ANCHOR.MIDDLE)
        text(s,x+I(0.85),y+I(0.12),I(2.0),I(0.3),nm,font=HEAD,size=9.5,bold=True,color=INK)
        text(s,x+I(0.85),y+I(0.38),I(2.0),I(0.26),role,size=8,color=MUT)
        if alma!="—": chip(s,x+I(0.85),y+I(0.6),I(1.6),I(0.26),alma,fill=BR_T,color=BRAND,size=6.5)
...
def bench(idx,total,wg):
    ...
    photo(s,os.path.join(H2,"gtiwari.png"),ML+I(0.14),y+I(0.14),I(0.74),I(0.74),fill=True,border=False,anchor=0.1)
    ...
    for i,(f,n,o) in enumerate(heads):
        ...
        photo(s,os.path.join(H2,f+".png"),x+I(0.64),yy,I(0.7),I(0.7),fill=True,border=False,anchor=0.1)
...
def wg5_body(wg,total,start):
    ...
    tiles=[("₹80,000+ Cr","held up across 10.46 lakh pending MACT cases nationally"),
           ...
           ("<5%","of hit-and-run victims actually receive compensation"),
    ...
    text(s,I(9.3),I(2.45),I(3.15),I(0.9),"<5%",font=HEAD,size=44,bold=True,color=BRAND)
    text(s,I(9.3),I(3.4),I(3.15),I(0.85),"of hit-and-run victims actually receive compensation under the scheme built for them.",size=11,color=INK,ls=1.3)
    text(s,I(9.3),I(4.35),I(3.15),I(1.0),"FY22-23: 205 claims filed against ~25,000 eligible crashes. Focused outreach lifted conversion to ~12% by FY23-24 — proof the pipeline responds.",size=9.5,color=MUT,ls=1.3)
...
def close(wg,idx,total):
    ...
    text(s,ML,I(2.05),I(11.5),I(1.4),f"We would be honoured to contribute\nto Working Group {wg}.",font=HEAD,size=30,bold=True,color=PAPER,ls=1.2)
    text(s,ML,I(3.7),I(10.5),I(0.6),"Our evidence, our district relationships, our technology and our expert bench are available to the Board — in whichever form is most useful.",size=12,color=LAVW,ls=1.4)
    hline(s,ML,I(4.55),I(5.2),color=RGBColor(0x6A,0x55,0xFF),wt=1.0)
    text(s,ML,I(4.75),I(8.6),I(0.85),
         '"We already know what is going wrong. What we need now is more responsibility, more discipline, more respect for life."',
         font=HEAD,size=13.5,bold=True,color=PAPER,ls=1.35,italic=True)
    text(s,ML,I(5.62),I(8),I(0.3),"MS Dhoni · Goodwill Ambassador, Crashfree India",size=10,color=LAVW)
    ...
    photo(s,a5("ph_dhoni_white.jpg"),I(9.6),I(2.05),I(3.1),I(4.2),fill=True,anchor=0.1,border=False)
    return s
...
def build_wg2():
    ...
    cover(2,"Safe Roads, Speeds & Vulnerable Road Users",
          [("31","sites audited across 18 cities"),("7","fixes fully implemented"),("25+","written authority approvals")],1,TOTAL)
    letter(2,
      ...
      2,TOTAL)
    agenda(2,"Project Rakshak in depth","17 – 25","26 – 27",3,TOTAL)
    about(4,TOTAL,2)
    approach(5,TOTAL,2)
    why_fit(2,6,TOTAL,
        ...
        smes=[("Prof. Geetam Tiwari","TRIP Centre, IIT Delhi · IRC H-7",os.path.join(H2,"gtiwari.png"),True),
              ("Abhay Damle","Ex-Joint Secretary, MoRTH",os.path.join(H2,"damle.png"),True),
              ("Vaibhav Dange","Ex-Advisor, MoRTH",os.path.join(H2,"dange.png"),True),
              ("Akhilesh Srivastava","ITS India · IRF",os.path.join(H2,"asrivastava.png"),True)])
    portfolio(7,TOTAL,2)
    satark(8,TOTAL,2)
    opendata(9,TOTAL,2)
    ...
    close(2,27,TOTAL)
    save(os.path.join(BUILD,"CFI_NRSB_WG2_v3.pptx"))

def build_wg5():
    new_deck(); TOTAL=26
    cover(5,"Post-Crash Care, Policy Standards & State Implementation",
          [("24/7","Aasha — AI legal companion for victims"),("100+","victims helped in helpdesks' first 4 days"),("4","District Road Safety Committees joined")],1,TOTAL)
    letter(5,
    ...
    why_fit(5,6,TOTAL,
        ...
        smes=[("Justice J.R. Midha","Retd., Delhi HC · MACT process architect",a6("pp_midha.png"),True),
              ("Piyush Tewari","SaveLIFE Foundation · Good Samaritan Law","PT",False),
              ("Dr. Maya Tandon","Sahayta Trust · trauma-response pioneer",os.path.join(H2,"tandon.png"),True),
              ("Mishika Singh","Neev Foundation · legal helpdesks",a6("pp_mishika.png"),True)])
    portfolio(7,TOTAL,5)
    satark(8,TOTAL,5)
    opendata(9,TOTAL,5)
    ...
    close(5,26,TOTAL)
    save(os.path.join(BUILD,"CFI_NRSB_WG5_v3.pptx"))
